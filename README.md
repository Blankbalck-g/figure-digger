                                                                    ...
                                                                    └─ 数据图 ─┬─ 矢量：直接读 PDF 路径 + 真实刻度文字（无损）
                                                                               │            └─ 读不出刻度 → 渲染成位图，转下一条路线
                                                                                └─ 位图：切分面板 → 图例取色 → VLM 读轴（OCR 默认关）
                                                                                              ↓
                    模型指出曲线 + 锚点 ──→ 代码从锚点精确追踪 ──→ VLM 抽查校验 ──→ CSV / 质检图 / 报告 / token
```
...
5. **曲线定位与追踪（模型负责懂、代码负责准）**：
   ...
   - **模型读图补缺口**：像素证据补不上时（隐藏段不是精确贴着另一条线、或曲线两头被压住），把那一小块**裁出来**、把已识别的部分画上去（别的曲线画灰），让模型沿两侧的走向给 2~3 个锚点，代码再把锚点**插回缺口**（只往缺口里插，已追到的部分不动）。每段一次小调用，只在"这条曲线明显比别人短、或中间有长断口"时才问；裁剪图存档在 `out/<论文>/gaps/`，报告里写 `🧠 模型读图补的锚点`
