...
5. **曲线追踪**：对每个系列颜色做色相+饱和度匹配，然后**按列连续性追踪**——每列取"离上一列最近的暗段"，并用最近几列估出斜率做预测。整列取中位数会在同色标记点、误差棒、标注文字处掉进两者的缝隙，产生假的震荡；连续性追踪不会。收尾再用滚动中位数剔掉短时偏移（例如线旁边同色的 "S ∝ t^0.5" 标注）
   - **同色多条线**：追完一条就把"线+它的标记符号（±一个标记半径的走廊）"整段擦掉再追下一条，所以同色两条曲线能分别出来，而不会把标记轮廓当成第二条线
   - **黑线**：默认不碰（坐标轴、网格、每个文字都是黑的）。模型判定"图里确实有一条深色数据线"时才启用：先删掉长直线（网格/坐标框）再追踪，并用"每列暗段长度"把误差棒串成的链挡掉
...
| 曲线可信度闸门（跨度过小/点太少/大跳变） | `dig/batch_extract.py` 顶部 `MIN_SPAN_X`、`MIN_POINTS`、`MAX_BIG_JUMPS`、`BIG_JUMP_FRAC`、`DARK_MIN_Y_SPAN` |
| 追踪连续性容差、同色擦除走廊宽度 | `dig/extract_lines.py` `_trace_mask` 的 `max_jump/max_gap`、`trace_series_instances` 的 `erase_width=45`、`strict_tol=8` |
| 图框与图例/放大子图的识别 | `dig/extract_lines.py` `find_boxes`（长直线组合+四边验真）、`batch_extract.inner_boxes` |
| 模型对每条线的角色判定（数据/切线/标注） | `dig/vlm_tasks.py` L43 `NAME_PROMPT` 的 `is_data`/`role`/`dark_series` 字段 |
