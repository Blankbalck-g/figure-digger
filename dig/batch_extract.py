...
def _unique_path(outdir, name):
    """Never let two series write to the same file.

    Two different colours can end up with the same hue name (violet / violet), and the
    second write silently replaced the first (measured on 2026-01-0340 p10_img1_p2:
    two series, one file). Suffix until the name is free.
    """
    p = Path(outdir) / name
    if not p.exists():
        return p
    ...
    for k in range(2, 100):
        cand = Path(outdir) / f"{stem}_{k}{suffix}"
        if not cand.exists():
            return cand
    return p


def process_objects(image_path, frame, objects, decisions, rng, outdir):
    """Write CSVs from the detected objects, following the model's per-object verdict.

    markers -> the centre of each marker glyph (no tracing, so the trace cannot wobble
    onto the connecting line or an error bar); line -> the continuity-traced path;
    is_data=false or output=skip -> nothing, but the reason is reported.
    """
    xmin, xmax, ymin, ymax = rng
    dec = {d.get("id"): d for d in (decisions or [])}
    series, skipped = [], []
    for o in objects:
        d = dec.get(o.get("id"))
        if d is None:
            continue
        want = str(d.get("output") or "").lower()
        ...
        with name.open("w", newline="", encoding="utf-8") as fh:
            wr = csv.writer(fh)
            wr.writerow(["x", "y"])
            for xv, yv in data:
                wr.writerow([f"{xv:.6g}", f"{yv:.6g}"])
        series.append({
            "file": name.name, "source": "object", "object": o.get("id"),
            "kind": o["kind"], "belongs_to": d.get("belongs_to"), "output": want,
            "reason": d.get("reason"), "color_hex": o.get("hex"),
            "points": len(data),
            "x_range": [round(min(p[0] for p in data), 4), round(max(p[0] for p in data), 4)],
            "y_range": [round(min(p[1] for p in data), 4), round(max(p[1] for p in data), 4)],
        })
